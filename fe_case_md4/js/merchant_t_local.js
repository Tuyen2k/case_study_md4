...
function displayMerchant(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (data[i].activity.id_activity === 2) {
            content += `<button class="btn btn-sm btn-primary" id="activity" onclick="activity(${data[i].id_merchant})"><i class="fa fa-check"></i></button>`
        } else if (data[i].activity.id_activity === 1){
            content += `<button class="btn btn-sm btn-primary" id="lock" onclick="lockMerchant(${data[i].id_merchant})"><i class="fa fa-times"></i></button>`
        }else {
            content += `<button class="btn btn-sm btn-primary" id="lock" onclick="activity(${data[i].id_merchant})"><i class="fa fa-question-circle"></i></button>`
        }
...
function lockMerchant(id_merchant) {
    let activity = {
        id_activity: 2
    }
    $.ajax({
